...
describe("Safe multisig governance (Phase 3)", () => {
  async function deployWithSafeFixture() {
    ...
    return { deployer, ownerB, ownerC, alice, usdc, vault, strategyManager, strategyA, strategyB, safe };
  }
  ...
  it("a real 2-of-3 Safe transaction can call setAllocations on StrategyManager", async () => {
    const { deployer, ownerB, strategyManager, strategyA, strategyB, safe } =
      await loadFixture(deployWithSafeFixture);

    const data = strategyManager.interface.encodeFunctionData("setAllocations", [
      [await strategyA.getAddress(), await strategyB.getAddress()],
      [7_000, 3_000],
    ]);

    // 2 trong 3 owner (deployer + ownerB) approve - đủ threshold=2.
    await expect(
      execSafeTransaction(safe, await strategyManager.getAddress(), data, [
        deployer.address,
        ownerB.address,
      ])
    ).to.not.be.reverted;
  ...
  it("a Safe transaction with only 1 of 3 approvals (below threshold) fails", async () => {
    const { deployer, strategyManager, strategyA, safe } = await loadFixture(deployWithSafeFixture);
    ...
    await expect(
      execSafeTransaction(safe, await strategyManager.getAddress(), data, [deployer.address])
    ).to.be.reverted;
  });
